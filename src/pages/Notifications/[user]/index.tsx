...
const UserNotifications: NextPage<{user: User}> = ({user}): JSX.Element => {
  ...
  const incomingTasks = data?.tasks?.filter((task: Task) => new Date(task.dateToComplete as Date) >= new Date());

  return (

    <div className={Styles.notificationPage}>
      <h1 className={Styles.title}>Incoming <span className='green'>Tasks</span></h1>
      {isSuccess && (
